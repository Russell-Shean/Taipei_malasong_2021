...
server <- function(input, output) {
    ...
    output$runner_info4 <- renderText({paste("net pace: ",taipei_combined_results[taipei_combined_results$Bib_number==as.character(input$bibnmbr),"min_per_mile_net_m"],"min",taipei_combined_results[taipei_combined_results$Bib_number==as.character(input$bibnmbr),"min_per_mile_net_s"],"secs per mile")})
    output$runner_info5 <- renderText({paste("race pace: ",taipei_combined_results[taipei_combined_results$Bib_number==as.character(input$bibnmbr),"min_per_mile_race_m"],taipei_combined_results[taipei_combined_results$Bib_number==as.character(input$bibnmbr),"min_per_mile_race_s"],"min/mile")})
