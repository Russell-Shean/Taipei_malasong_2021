...
page_1 <- read_html(url_list[1]) %>% html_elements("tr") %>% html_text2()


#Next create a new url for each page of the table and store in a vector
for(i in 1:14){
  url_list <- c(url_list, paste0("https://www.championchiptw.com/report/report_w.php?EventCode=20211219&Race=MA&sn=244&Sex=N&CatId=0&Cat&pagenum=2",i))
}
...
for(i in url_list[1:13]){
  page_x <- read_html(i) %>% html_elements("td:El") %>% html_text2()
...
write.csv(marathon_df, file = "taipei_marathon_2021_results.csv")
